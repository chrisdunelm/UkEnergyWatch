...
<GENERATION_BY_FUEL_TYPE_TABLE>
 ...
 <LAST_UPDATED AT="2014-04-15 06:00:00"></LAST_UPDATED>
  ...
  it should "not import if too soon" in prepare { implicit session =>
    val t0 = t(2014, 4, 15, 5, 0)
    val t1 = t(2014, 4, 15, 5, 5)
    GenByFuelsLive += GenByFuel("CCGT", t0, t1, 1000)
...
<GENERATION_BY_FUEL_TYPE_TABLE>
 ...
 <LAST_UPDATED AT="2014-04-15 06:00:00"></LAST_UPDATED>
      ...
      """.trim
    TestImporter.clock.set(new DateTime(2014, 4, 15, 5, 9, DateTimeZone.UTC))
    TestImporter.httpFetcher.set("http://www.bmreports.com/bsp/additional/saveoutput.php?element=generationbyfueltypetable&output=XML", s)
    TestImporter.run(Importer.ImportLiveGenByFuel)
  ...
  it should "delete data older than 24 hours" in prepare { implicit session =>
    GenByFuelsLive += GenByFuel("CCGT", t(2014, 4, 14, 4, 55), t(2014, 4, 14, 5, 0), 1000)
    val t0 = t(2014, 4, 14, 5, 0)
    val t1 = t(2014, 4, 15, 5, 5)
    GenByFuelsLive += GenByFuel("CCGT", t0, t1, 2000)
...
<GENERATION_BY_FUEL_TYPE_TABLE>
 ...
 <LAST_UPDATED AT="2014-04-15 06:00:00"></LAST_UPDATED>
      ...
      """.trim
    TestImporter.clock.set(new DateTime(2014, 4, 15, 5, 2, DateTimeZone.UTC))
    TestImporter.httpFetcher.set("http://www.bmreports.com/bsp/additional/saveoutput.php?element=generationbyfueltypetable&output=XML", s)
    TestImporter.run(Importer.ImportLiveGenByFuel)

    GenByFuelsLive.list.id0.toSet shouldBe Set(
      GenByFuel("CCGT", t0, t1, 2000)
    )
